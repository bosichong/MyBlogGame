...
var savings_balance: float:
    get:
        return GlobalDataManager.get_bank().savings_balance if GlobalDataManager else 0
    set(value):
        if GlobalDataManager:
            GlobalDataManager.get_bank().savings_balance = value

# 活期存款利息未结算（玩家存入后 money 减少，取出后 money 增加）
var savings_balance_interest: float:
    get:
        return GlobalDataManager.get_bank().savings_interest if GlobalDataManager else 0
    set(value):
        if GlobalDataManager:
            GlobalDataManager.get_bank().savings_interest = value
...
var fixed_deposits: Dictionary:
    get:
        return GlobalDataManager.get_bank().fixed_deposits if GlobalDataManager else {}
    set(value):
        if GlobalDataManager:
            GlobalDataManager.get_bank().fixed_deposits = value

# 下一个定期存款的唯一ID（自增）
var next_deposit_id: int:
    get:
        return GlobalDataManager.get_bank().next_deposit_id if GlobalDataManager else 1
    set(value):
        if GlobalDataManager:
            GlobalDataManager.get_bank().next_deposit_id = value
