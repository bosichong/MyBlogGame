...
const MAX_LEVEL = 1
## 写作能力：决定文章质量和写作速度。
var writing_ability: int = 5

## 技术能力：影响博客维护、SEO优化和网站性能。
var technical_ability: int = 5

## 编程能力：可以撰写编程相关的博文，2级学完可以开始接私活赚钱。
var code_ability = 0

## 文学能力：可以博主的文学素养，除了可以撰写相关文章，还可以优化其他类型的文章。
var literature_ability = 0

## 绘画能力:最后可以为原画师和大画家。
var drawing_ability = 0
...
## 文章被分享后的访问量加
func update_rss():
	blog_data.rss = int(blog_data.views / 2000)
	
## 章的收藏增加
func update_favorites(views):
	blog_data.rss = int(views / 100)

## 根据文章的收藏量，随机返回每日增加的文章的问量	
func favorites_add(v):
	return randi_range(0,v)

## 添加新的博客文章
func add_new_blog_post(title: String, category: String) -> Dictionary:
	...
	var new_post: Dictionary = {
		...
		"is_money":false, #是否收费
		"date" : blog_date,
		"quality": Utils.get_quality(category),
...
func simulate_new_blog_post(category) -> int:
	...
	if stamina > d.stamina and category: ## 如果体力大于，并且当天有写作任务。
		...
		var new_post = add_new_blog_post(new_title, category)
...
func update_blog_views() -> int:
	...
	for post in blog_data["posts"]:
		...
		# 根据文章的收藏增加访问量
		end_views_for_post += favorites_add(post.favorites)
		...
		# 叠加访问量后更新每篇文章的收藏.
		blog_data.favorites = update_favorites(end_views_for_post)
...
func maintain_website_seo(category: String) -> int:
	var d = Utils.find_category_by_name( Utils.website_maintenance,category)
	if stamina < d.stamina :
		emit_signal("signal_website_seo_no_stamina","体力不足，无法进行网站维护！")
		stamina += Utils.add_property(stamina,5)
		return 0
	stamina -= d.stamina #消耗体力值
