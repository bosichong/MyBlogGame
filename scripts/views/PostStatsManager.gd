...
## 记录文章每日访问量
func record_post_views(post_id, views: int, sources: Dictionary) -> void:
...
## 初始化文章统计
func _init_post_stats(post_id) -> void:
    post_stats[post_id] = {
        "total_views": 0,
        "daily_views": [],
        "weekly_views": [],
...
## 获取文章总访问量
func get_post_total_views(post_id) -> int:
    var stats = post_stats.get(post_id, {})
    return stats.get("total_views", 0)

## 获取文章每日访问量趋势
func get_post_daily_views(post_id, days: int = 30) -> Array:
    var stats = post_stats.get(post_id, {})
    var daily = stats.get("daily_views", [])
    return daily.slice(-days)

## 获取文章来源统计
func get_post_sources(post_id) -> Dictionary:
    var stats = post_stats.get(post_id, {})
    return stats.get("sources", {})

## 更新周统计
func update_weekly_stats(post_id) -> void:
    if not post_stats.has(post_id):
        return
...
## 更新月统计
func update_monthly_stats(post_id) -> void:
    if not post_stats.has(post_id):
        return
...
func get_top_posts(limit: int = 10) -> Array:
    var result = []
    for pid in post_stats:
        result.append({
            "post_id": pid,
            "total_views": post_stats[pid].total_views
        })
